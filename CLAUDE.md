...
- Deferred (not started): token/voice metering, SSO, OAuth refresh, semantic memory recall, internal event emitters, analytics charts, publisher-marketplace+commission, logo upload, email invites, audit log, enforce mfa/session/retention, AiEmployee.departmentId FK.
- Run all e2e: from `apps/api`, `pnpm test` with `SKILL_EXECUTOR=mock BILLING_PROVIDER=mock ENCRYPTION_KEY=<64hex> LLM_PROVIDER=mock EMBEDDINGS_PROVIDER=hash STORAGE_PROVIDER=local` + `DATABASE_URL`+`REDIS_URL`+JWT secrets (currently **121/121**, 19 suites).
