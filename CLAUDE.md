...
- Run e2e: from `apps/api`, `pnpm test` with `SKILL_EXECUTOR=mock BILLING_PROVIDER=mock ENCRYPTION_KEY=<64hex> LLM_PROVIDER=mock EMBEDDINGS_PROVIDER=hash STORAGE_PROVIDER=local` + `DATABASE_URL`+`REDIS_URL`+JWT secrets (currently **155 e2e tests, 24 suites** + **34 unit** via `pnpm run test:unit`; jest-e2e runs serial `maxWorkers:1`). NOTE: 2 of the 155 e2e tests currently fail in this local environment for reasons unrelated to any module's code — `integrations.e2e-spec.ts`'s OAuth-UNCONFIGURED test fails because `apps/api/.env` has real `OAUTH_GOOGLE_CLIENT_ID/SECRET` set locally, and `knowledge.e2e-spec.ts`'s search-score assertion is sensitive to local embedding-score variance; both reproduce identically in isolation with no other files changed.
