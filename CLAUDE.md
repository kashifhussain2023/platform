...
- ⬜ Next (canonical Steps 1–15 flow) — LOOPING through these until all covered: **Analytics/KPI dashboard (12)** · **Billing/Stripe + default subscription (1,13)** · **Marketplace expansion (14)** · **Event triggers + Test→Activate (8,9,11)** · **Continuous Learning (15)** · (real OAuth flows deferred).
- Run all e2e: from `apps/api`, `pnpm test` with `LLM_PROVIDER=mock EMBEDDINGS_PROVIDER=hash STORAGE_PROVIDER=local` + `DATABASE_URL`+`REDIS_URL`+JWT secrets (currently 49/49).
