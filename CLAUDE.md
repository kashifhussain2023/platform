...
- ⬜ Next (canonical Steps 1–15 flow): **Analytics/KPI dashboard (12)** · **Integrations/OAuth connectors (4-real, 6)** · **Billing/Stripe + default subscription (1,13)** · **Marketplace expansion (14)** · **Test→Activate workflow stages + scheduled/webhook triggers (8,9)** · **Continuous Learning (15)**.
- Run all e2e: from `apps/api`, `pnpm test` with `LLM_PROVIDER=mock EMBEDDINGS_PROVIDER=hash STORAGE_PROVIDER=local` + `DATABASE_URL`+`REDIS_URL`+JWT secrets (currently 42/42).
