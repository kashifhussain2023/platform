...
- Run all e2e: from `apps/api`, `pnpm test` with `LLM_PROVIDER=mock EMBEDDINGS_PROVIDER=hash STORAGE_PROVIDER=local BILLING_PROVIDER=mock` + `DATABASE_URL`+`REDIS_URL`+JWT secrets (currently **76/76**, 13 suites).
