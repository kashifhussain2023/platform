...
- **P0/P1 gap-remediation in progress** (impl loop): ✅ Unit1 **RBAC + Users** (RolesGuard/@Roles OWNER>ADMIN>MEMBER, `modules/users`, `/team`, User.status, admin mutations gated). ⬜ Unit2 creds encryption · Unit3 real executors+OAuth+Stripe webhooks (behind existing interfaces; mock stays tested default) · Unit4 workflow APPROVAL node · Unit5 employee goals+KPI targets · Unit6 security-policy/team/department. Company=org=workspace (decided, no split).
- Run all e2e: from `apps/api`, `pnpm test` with `LLM_PROVIDER=mock EMBEDDINGS_PROVIDER=hash STORAGE_PROVIDER=local BILLING_PROVIDER=mock` + `DATABASE_URL`+`REDIS_URL`+JWT secrets (currently **83/83**, 14 suites).
