...
- ⬜ Next (from the Steps 2–15 spec): **Approval Center (12)** · **Ops Dashboard + Reports/KPIs (10,13)** · **Integrations/OAuth (6)** · **Billing/Stripe (15)** · **Employee-template marketplace (14)**.
- Run all e2e: from `apps/api`, `pnpm test` with `LLM_PROVIDER=mock EMBEDDINGS_PROVIDER=hash STORAGE_PROVIDER=local` + `DATABASE_URL`+`REDIS_URL`+JWT secrets (currently 37/37).
